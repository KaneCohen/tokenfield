 ...
 * tokenfield 0.2.3 <https://github.com/KaneCohen/tokenfield>
    ...
    v.xhr = ajax(reqData, o.remote, () => {
      if(v.xhr.readyState == 4) {
        if(v.xhr.status == 200) {
          ...
          let items = this._filterData(val, response);
  ...
  _keyInput(e) {
    ...
    if (typeof v.cache[val] === 'undefined') {
      ...
      if (o.remote.url) {
        ...
        let items = this._filterData(val, o.items);
        v.suggestedItems = this._filterSetItems(items);
        this.showSuggestions();
      }
    } else {
      // work with cache data
      let items = this._filterData(val, v.cache[val]);
      v.suggestedItems = this._filterSetItems(items);
      this.showSuggestions();
    }
  }
