    ...
    v.xhr = ajax(reqData, o.remote, () => {
      if(v.xhr.readyState == 4) {
        if(v.xhr.status == 200) {
          ...
          let mappedData = this.remapData(response);
          let items = this._filterData(val, mappedData);
  ...
  _keyInput(e) {
    ...
    if (typeof v.cache[val] === 'undefined') {
      ...
      if (o.remote.url) {
        ...
        let mappedData = this.remapData(o.items);
        let items = this._filterData(val, mappedData);
        v.suggestedItems = this._filterSetItems(items);
        this.showSuggestions();
      }
      ...
      let mappedData = this.remapData(v.cache[val]);
      let items = this._filterData(val, mappedData);
      v.suggestedItems = this._filterSetItems(items);
      this.showSuggestions();
    }
