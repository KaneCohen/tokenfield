    ...
    v.xhr = ajax(reqData, o.remote, () => {
      if(v.xhr.readyState == 4) {
        if(v.xhr.status == 200) {
          ...
          let mappedData = this._mapData(response);
          let remappedData = this.remapData(mappedData);
          let items = this._filterData(val, remappedData);
  ...
  _keyInput(e) {
    ...
    if (typeof v.cache[val] === 'undefined') {
      ...
      if (o.remote.url) {
        ...
        let mappedData = this._mapData(o.items);
        let remappedData = this.remapData(mappedData);
        let items = this._filterData(val, remappedData);
        v.suggestedItems = this._filterSetItems(items);
        this.showSuggestions();
      ...
      let mappedData = this._mapData(v.cache[val]);
      let remappedData = this.remapData(mappedData);
      let items = this._filterData(val, remappedData);
      v.suggestedItems = this._filterSetItems(items);
      this.showSuggestions();
