 ...
 * tokenfield 0.6.0 <https://github.com/KaneCohen/tokenfield>
...
class Tokenfield extends EventEmitter {
  ...
  _keyAction(e) {
    ...
    switch (keyName) {
      ...
      case 'delete':
        this._abortXhr();
        if ((! html.input.selectionStart && e.keyCode === 8) ||
  ...
  _focusItem(key, shift = false, ctrl = false, add = false) {
    ...
    if (shift) {
      ...
      if (target === 0 && first === null && last === null) {
        return;
      } else if (first === 0 && target === this._vars.setItems.length - 1 && add) {
        return;
      } else if (first === null && last === null) {
        this._vars.setItems[target].focused = true;
      } else if (target === 0 && last === this._vars.setItems.length - 1 && ! add) {
